...
__global__ void check_tiles_touched(const uint32_t* a, int P, int* out_max, int* out_min, int* out_bad) {
    __shared__ int smax, smin, sbad;
    if (threadIdx.x == 0) { smax = INT_MIN; smin = INT_MAX; sbad = 0; }
    __syncthreads();

    for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < P; i += blockDim.x * gridDim.x) {
        int v = a[i];
        atomicMax(&smax, v);
        atomicMin(&smin, v);
        if (v < 0 || v > 100000) atomicAdd(&sbad, 1); // 阈值你可改，比如 > H*W*D
    }
    __syncthreads();
    if (threadIdx.x == 0) {
        atomicMax(out_max, smax);
        atomicMin(out_min, smin);
        atomicAdd(out_bad, sbad);
    }
}
	...
	bool debug)
{
	...
	if (debug) {
		printf("H=%d W=%d D=%d P=%d C=%d N=%d\n", H, W, D, P, C, N);
	}
	...
	if (debug) {
		printf("finished InclusiveSum\n");

		int hmax = INT_MIN, hmin = INT_MAX, hbad = 0;
		int *dmax, *dmin, *dbad;
		cudaMalloc(&dmax, sizeof(int));
		cudaMalloc(&dmin, sizeof(int));
		cudaMalloc(&dbad, sizeof(int));
		cudaMemcpy(dmax, &hmax, sizeof(int), cudaMemcpyHostToDevice);
		cudaMemcpy(dmin, &hmin, sizeof(int), cudaMemcpyHostToDevice);
		cudaMemcpy(dbad, &hbad, sizeof(int), cudaMemcpyHostToDevice);

		check_tiles_touched<<<256, 256>>>(geomState.tiles_touched, P, dmax, dmin, dbad);
		cudaDeviceSynchronize();

		cudaMemcpy(&hmax, dmax, sizeof(int), cudaMemcpyDeviceToHost);
		cudaMemcpy(&hmin, dmin, sizeof(int), cudaMemcpyDeviceToHost);
		cudaMemcpy(&hbad, dbad, sizeof(int), cudaMemcpyDeviceToHost);
		printf("[tiles_touched] min=%d max=%d bad=%d\n", hmin, hmax, hbad);
	}
	...
	if (debug) {
		printf("Num rendered instances: %d\n", num_rendered);
	}
