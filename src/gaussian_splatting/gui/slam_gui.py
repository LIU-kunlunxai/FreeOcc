...
class SLAM_GUI:
    ...
    def init_widget(self):
        ...
        self.window = gui.Application.instance.create_window("FreeOcc", self.window_w, self.window_h)
    ...
    def receive_data(self, q):
        ...
        if gaussian_packet.has_gaussians:
            ...
            # Robustly compute number of gaussians (get_xyz is expected to be [N,3],
            # but can be empty or have unexpected dims early on).
            try:
                xyz = self.gaussian_cur.get_xyz
                n_gauss = int(xyz.shape[0]) if hasattr(xyz, "shape") and xyz.ndim >= 1 else 0
            except Exception:
                n_gauss = 0
            self.output_info.text = "Number of Gaussians: {}".format(n_gauss)
            self.init = True
